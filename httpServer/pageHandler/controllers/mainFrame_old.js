...
log4js.configure({appenders: RedisConfig.logAppenders});
...
var getSqlStatementBy_IpPort_Type_Ver_IDC = function ( )
{
	...
	if( _condition.hasOwnProperty("playType") ){
	    ...
	    sql_para2 = sql_para2 + strSqlType;
			...
			logs.logger.error("playType error!");	
		}
	}
	...
	if( _condition.hasOwnProperty("primary") ){
	    ...
	    sql_para2 = sql_para2 + strSqlType;
			...
			logs.logger.error("primary error!");	
		}
	}
	...
	if( _condition.hasOwnProperty("net_type") ){
		if ( _condition.net_type && _condition.net_type.length == 2  && Number(_condition.net_type[0]) != -1) {		
			...
			logs.logger.error("net_type error!");	
		}
	}
	...
	if( _condition.hasOwnProperty("upgrade_type") ){
		if ( _condition.upgrade_type && _condition.upgrade_type.length == 2  && Number(_condition.upgrade_type[0]) != -1) {		
			...
			logs.logger.error("upgrade_type error!");	
		}
	}
	...
	if( _condition.hasOwnProperty("splatid") ){
		if ( _condition.splatid && _condition.splatid.length == 2  && Number(_condition.splatid[0]) != -1) {		
			...
			logs.logger.error("splatid error!");	
		}
	}
...
/*
	if( _condition.hasOwnProperty("table_live") ) {	
		if ( _condition.table_live && _condition.table_live.length==2 ) {
			...
			res.data = new Array();;
			res.result = "success";
			select_version(function(res_ver){
				if(res_ver && res_ver.data){
					for(var n = 0; n < res_ver.data.length; n++)
					{
						for(var j = 0;j < dbcfg.table_live.length; j++)
						{
							if(dbcfg.table_live[j].value == res_ver.data[n][1])
							{
								table_version = util.format(" %d ",res_ver.data[n][0]);
								live_table[j] = res_ver.data[n][0];
								if(j < dbcfg.table_live.length-1)
								{
									table_version = ","+table_version;
								}
								table_collect = table_version + table_collect;
							}
						}
					}
					strSqlType = util.format(" and "+_tableAlias+".version in \( ");
					sql_para2 = sql_para2 + strSqlType + table_collect+"\)";
					
					strDuration = _tableAlias+".version";
					var fromDateFMT = fromDate.getFullYear() + "-" + (fromDate.getMonth() + 1) + "-" + fromDate.getDate();
					...
					//t_0_0.time_stamp >= date_format('2014-7-1 20','%Y-%m-%d %H') and t_0_0.time_stamp < date_format('2014-7-1 22','%Y-%m-%d %H') 
						unit_time = util.format(" %s.%s >= date_format('%s','%%Y-%%m-%%d %H') and %s.%s <= date_format('%s','%%Y-%%m-%%d %H')",
										_tableAlias,_tbTimeDesc,start_time,_tableAlias,_tbTimeDesc,end_time); 
						if(i != 0)
						{
		...
		if ( _condition.table_vod && _condition.table_vod.length==2 ) {
			...
			select_version(function(res_ver){
				if(res_ver && res_ver.data){
					...
					for(var n = 0; n < res_ver.data.length; n++)
					{
						for(var j = 0;j < dbcfg.table_vod.length; j++)
						{
							if(dbcfg.table_vod[j].value == res_ver.data[n][1])
							{
								table_version = util.format(" %d ",res_ver.data[n][0]);
								vod_table[j] = res_ver.data[n][0];
								if(j < dbcfg.table_vod.length-1)
								{
									table_version = ","+table_version;
								}
								table_collect = table_version + table_collect;
...
/*
	...
	*/
	...
	if(_condition.hasOwnProperty("table_webp2p")) {
		...
		{
			logger.error("table_webp2p error!");	
		}
	}
	if( _condition.hasOwnProperty("table_termid") || _condition.hasOwnProperty("table_webp2p")){		
		...
		if(toDate.getMonth() != fromDate.getMonth())
		{
			for( var i=0;i<Number(diffDay-toDate.getDate()+1);i++)
			{
				...
				//console.log("next date: "+start_time+"        "+end_time);
				if(i != 0)
				{
					sql_para = sql_para+" or "+unit_time ;
				}
			...
			{
			//	console.log("unit_time: "+unit_time);
		...
		/*
			sql = "select "+
			"DATE_FORMAT("+_tableAlias+"."+_tbTimeDesc+",'%Y-%m-%d') "+_tbTimeDesc+", "+
			_selectParam+
			" from " +_tableName +" " + _tableAlias +
			" where  ("+sql_para+")"+sql_para2+" group by DATE("+_tableAlias+"." + _tbTimeDesc + ")"+","+strDuration;
			*/
			sql = "select DATE_FORMAT("+select_ten_minute+" , "+_selectParam +" from "+_tableName+ " where "+date_time+ sql_para2+ " group by show_time_segment ,"+strDuration;
		}else
		{
			date_time =  util.format("( time_stamp >= date_format(' %s %s:00:00 ', '%%Y-%%m-%%d %%H:%%i:%%s') and time_stamp <= date_format('%s %s:%s:00','%%Y-%%m-%%d %%H:%%i:%%s') ) ",fromDateFMT,fromHour,toDateFMT,toHour,toMinute);
		/*
			sql = "select DATE_FORMAT("+_tableAlias+"."+_tbTimeDesc+",'%Y-%m-%d') "+_tbTimeDesc+", "+
			_selectParam+
			" from "+_tableName+" "+ _tableAlias +
			" where  ("+sql_para+")"+sql_para2+" group by DATE_FORMAT("+_tableAlias+"."+_tbTimeDesc+",'%Y-%m-%d %H')"+","+strDuration;
	...
	if( _condition.hasOwnProperty("table_p2p_all")||  _condition.hasOwnProperty("table_p2p_web") ||  _condition.hasOwnProperty("table_p2p_cde")) {			
		...
		/*
			...
			_selectParam+
			" from " +_tableName +" " + _tableAlias +
			" where  ("+sql_para+")"+table_p2p+" group by DATE("+_tableAlias+"." + _tbTimeDesc + ")";
			*/
		...
		/*
			...
			_selectParam+
			" from "+_tableName+" "+ _tableAlias +
			" where  ("+sql_para+")"+table_p2p+" group by DATE_FORMAT("+_tableAlias+"."+_tbTimeDesc+",'%Y-%m-%d %H')";
			*/
			sql = "select DATE_FORMAT("+select_ten_minute+" , "+_selectParam +" from "+_tableName+ " where "+date_time+ table_p2p+ " group by show_time_segment ;";
		}
		callback(sql);				
...
var select_version = function(callback)
{
	...
	db.getClient().query(sql, function(err, rows) {
    	if (!err) 
    	{
    	 	if (!rows.length) {
				res_ver.result = "zero"; 
				...
				rows.forEach(function( _value ) {
					if( 0 == Number(_value['ver_del']) ){        			
						...
						//console.log("===res_ver: "+JSON.stringify(res_ver.data));
					}
			...
			console.log(" select_version error !");
			if(callback){
				callback("error");
			}
		}
			...
			if (!err) {
				if( rows.length>=1 ){
					if( rows[0]['ver_del'] == 1)
					{
						...
						db.getClient().query(ver_sql,function(err) {
							if(!err)
							{
			...
			if (!err) {
				if( rows.length>=1 ){
					if( rows[0]['g_del'] == 1)
					{
						...
						logs.logger.info("update"+sql);
						db.getClient().query(groupid_sql,function(err) {
							if(!err)
							{
			...
			if (!err) {
				if( rows.length>=1 ){
					if( rows[0]['s_del'] == 1)
					{
						...
						logs.logger.info("update"+sql);
						db.getClient().query(streamid_sql,function(err) {
							if(!err)
							{
			...
			if (!err) {
				if( rows.length>=1 ){
					if( rows[0]['a_del'] == 1)
					{
						...
						logs.logger.info("update"+sql);
						db.getClient().query(upgradeid_sql,function(err) {
							if(!err)
							{
...
exports.queryCDE = function(args) {
	...
	if (sess) {
		...
		/*
		for(var id=0; id<overLoadData.length; id++)
		{
			if(overLoadData[id].idc == 0)
			{
				CDE_idc = "电信";
			}
			else if(overLoadData[id].idc == 1)
			{
				CDE_idc = "联通";	
			}
			else
			...
			res.data.push([overLoadData[id].ser_ip, overLoadData[id].ser_port, CDE_idc, (overLoadData[id].cde || 0)]);
		}
		*/
  		thisObj.responseDirect(200, "text/json", JSON.stringify(res));
	}else {
...
exports.queryRtmfp = function(args) {
	...
	if (sess) {
		...
		for(var id=0; id<overLoadData.length;id++)
		{
			...
			res.data.push([overLoadData[id].ser_ip, overLoadData[id].ser_port, rtmfp_idc, (overLoadData[id].rtmfp||0)]);
		}
		*/
		thisObj.responseDirect(200, "text/json", JSON.stringify(res));
	}else {
...
{
			//console.log("mainFrame queryRecentAllInfo:" + rtmfp +" : "+ cde );
	var thisObj = this;
	var res = {};
	res.result = "success";
	...
	if (sess) 
	{
		...
		for( var shareRateDataID = 0; shareRateDataID < shareRateData.length ;shareRateDataID++ ){
			...
			Number(shareRateData[shareRateDataID]['lpsize']);
  ...
  if (sess) 
  {
	...
	var _selectParam = arguments[3];//����ʱ���ѯ���ֶ�����
  	...
  	TABLE_RTMFP_SERVERDATA,//��ݱ���
		...
		logs.logger.info( "onlineuser sql:"+sql ); 
	//	console.log("mainFrame user_online sql:"+sql);
		db.getClient().query(sql, function(err, rows) {
    	 if (!err) 
    	 {
	    	 	if (!rows.length) {
...
exports.query_hisdata_nodedistribute = function(args) 
{
	...
	if (sess) 
	{
		...
		//console.log("mysql:"+mysql);		
		 ...
		 db.getClient().query(mysql, function(err, rows) {		
...
exports.query_hisdata_ptopratio = function(args) {
  ...
  var res = {};
	...
	if (sess) {
		...
		"sum( cde_mo_size) cde_mo_size, sum( lpsize) lpsize ,sum(up_rtmfp) up_rtmfp,sum(up_cde) up_cde,"+
	        ...
	        else 
	        {
				...
				rows.forEach(function( _value ) {
              	var index = i;
					...
					var allSize = cdn + pc + tv + box + mb + cde_pc + cde_tv + cde_box + cde_mb +lp;
					...
					var video_rate = allSize*8*1024*1024/rep_count/60;  //allSize*8 *1024*1024/时间/在线人数
					 ...
					 (((pc+cde_pc)/(pc + tv + box + mb + cde_pc + cde_tv + cde_box + cde_mb+lp)*100).toFixed(1))+'%',                    //pc贡献比
					 (((cde_pc+cde_tv+cde_box+cde_mb)/(pc + tv + box + mb + cde_pc + cde_tv + cde_box + cde_mb+lp)*100).toFixed(1))+'%'  //websocket贡献比
					 ]
	            });
	        }
    	 }else{
...
exports.query_hisdata_stagequality = function(args) {
	...
	if (sess) {
		...
		logs.logger.info("useTable:"+useTable);
		...
		"sum( act_time_twelve) act_time_twelve";
...
exports.query_hisdata_filmduration = function(args) {
	...
	if (sess) {
		...
		"sum( p2p_mo_size) p2p_mo_size, sum( cde_pc_size) cde_pc_size ,"+
		"sum( cde_tv_size) cde_tv_size, sum( cde_box_size) cde_box_size ,"+
		"sum( cde_mo_size) cde_mo_size, sum( lpsize) lpsize ,"+
...
*/
				...
				rows.forEach(function( _value ) {
					...
					var strDuration = (_value['show_time_segment']);
	            	var strDateTime = (_value['time_stamp']);
				...
				//		console.log("res.data: "+JSON.stringify(arr));
						res.data[index] = arr;
						tmp_sum = 0;
					}
					switch(strDuration)
		...
		db.getClient().query(mysql, function(err, rows) {		
			...
			if (!err)
			{
				...
				else 
				{
					...
					//	console.log("tmpDateTime: "+tmpDateTime+"    strDateTime:"+strDateTime+"    i:"+i);
						...
						if(tmpDateTime != strDateTime && tmp_count != 0){
							...
							arr.unshift(tmpDateTime);
							//console.log("0:"+tmp_arr[0]+"  1:"+tmp_arr[1]+"  2:"+tmp_arr[2]+"  3:"+tmp_arr[3]+"  4:"+tmp_arr[4]+"  5:"+tmp_arr[5]+"  6:"+tmp_arr[6]+"  7:"+tmp_arr[7]);
					//		logs.logger.info("res.data: "+JSON.stringify(arr));
					//		console.log("res.data: "+JSON.stringify(arr));
							res.data[index] = arr;
							tmp_sum = 0;
...
/* UTP 直播
exports.query_table_live = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		for(var idcId = 0; idcId < idc_list.length; idcId++ )
		{
			useTable.push(tb_prefix+"liv_"+idc_list[idcId]);
			useTableAlias.push("t_"+idcId);
		}
		
		if(useTable.length == 0)
		{
			logs.logger.error("query_table_live->tables error")
		}
		
		var selectElement = "sum( cdn_size) cdn_size ,sum( p2p_pc_size) p2p_pc_size ,"+
		"sum( p2p_tv_size) p2p_tv_size, sum( p2p_box_size) p2p_box_size ,"+
		"sum( p2p_mo_size) p2p_mo_size, sum( lpsize) lpsize ,"+
		"sum( share_rep_count)share_rep_count,  version";
		...
		var tmp_count = 0 ;
		var tmp_size = 0;
		var mysql = getRport_forms_IpPort_Type_Ver_IDC (
		useTable,
		useTableAlias,
		"T_m",
		{
			...
			"_toDate":args["to"],
			"_fromDate":args["from"],
			"table_live":[args["table_live"],"table_live"]
		},
		selectElement,
		"time_stamp",
		function(mysql){
			 ...
			 if (!err) 
			{
				...
				else 
				{
					...
					rows.forEach(function( _value ) {
						...
						var strDateTime = (_value['show_time_segment']);
						var rep_count = Number(_value['share_rep_count'])||0;
						var version = Number(_value['version'])||0;
						var cdn = isNaN(Number(_value['cdn_size']))?0:Number(_value['cdn_size']);
						var pc = isNaN(Number(_value['p2p_pc_size']))?0:Number(_value['p2p_pc_size']);
						var tv = isNaN(Number(_value['p2p_tv_size']))?0:Number(_value['p2p_tv_size']);
						var box = isNaN(Number(_value['p2p_box_size']))?0:Number(_value['p2p_box_size']);
						var mb = isNaN(Number(_value['p2p_mo_size']))?0:Number(_value['p2p_mo_size']);
						var lp = isNaN(Number(_value['lpsize']))?0:Number(_value['lpsize']);
						var allSize = cdn + pc + tv + box + mb+lp;
						var cdnBandwidth = cdn*8/(1024 * 1024 * 1024*3600);
						var saveBandwidth = (allSize - cdn)*8/(1024 * 1024 * 1024*3600);
						
						if(tmpDateTime != strDateTime && tmp_count != 0){
							i++;
							//arr=tmp_arr[0][0].concat(tmp_arr[1][0]).concat(tmp_arr[2][0]).concat(tmp_arr[0][1]).concat(tmp_arr[1][1]);//.concat(tmp_arr[2][1]).concat(tmp_arr[0][2]).concat(tmp_arr[1][2]).concat(tmp_arr[2][2]);
							res.data[index] = [tmpDateTime];
							for(var n = 0; n < dbcfg.table_live.length; n++)
							{
								//all_count = Number(all_count)+isNaN(Number(tmp_arr[n][2]))?0:Number(tmp_arr[n][2]);
								all_count = Number(all_count)+Number(tmp_arr[n][2]);
							}
							for(var m =0;m<3;m++)
							{
								for(var j = 0; j <dbcfg.table_live.length ;j++)
								{
									if(m != 2)
									{
										res.data[index].push(tmp_arr[j][m]);
									}
									else {
										res.data[index].push((100*(tmp_arr[j][2]/all_count)).toFixed(2)+"%");
									}
								}
							}
							s = 0;
						...
						if(args["table_live"] == 1)
						{
						//console.log("version: "+version+ "  live_table: "+live_table[s]);
							tmp_count = 1;
							switch(version)
							{
								case live_table[s]:
									tmp_arr[s] = new Array();
									tmp_arr[s].push((allSize == 0?0.00:(100 * (allSize - cdn)/allSize)).toFixed(2)+"%");
									tmp_arr[s].push((cdnBandwidth*1.488).toFixed(2));
									tmp_arr[s].push((rep_count/60).toFixed(0));
									break;			
							}
							s++;
						}						
					...
					if(tmp_arr.length>0)
					{
						...
						for(var m =0;m<3;m++)
						{
							for(var j = 0; j <dbcfg.table_live.length ;j++)
							{
								if(m != 2)
								{
									arr_last.push(tmp_arr[j][m]);
								}
								else {
									arr_last.push((100*(tmp_arr[j][2]/all_count)).toFixed(2)+"%");
								}
							}
						}				
						arr_last.unshift(tmpDateTime);
						res.data[i] = arr_last;
					}
				}			
			 }else{
				logs.logger.error("query table_live: "+err);
				res.result = "failed";
			 }
			 logs.logger.info("result:"+JSON.stringify( res ));
			 thisObj.responseDirect(200, "text/json", JSON.stringify(res));
...
*/
/*  UTP 点播部分
exports.query_table_vod = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		for(var idcId = 0; idcId < idc_list.length; idcId++ )
		{
			useTable.push(tb_prefix+"vod_"+idc_list[idcId]);
			useTableAlias.push("t_"+idcId);
		}

		if(useTable.length == 0)
		{
			logs.logger.error("query_table_vodtables error")
		}
		...
		"T_m",
		{
			...
			"_toDate":args["to"],
			"_fromDate":args["from"],
			"table_vod":[args["table_vod"],"table_vod"]
		},
		selectElement,
		"time_stamp",
		function(mysql){
			...
			db.getClient().query(mysql, function(err, rows) {		
				...
				if (!err) 
				{
					if (!rows.length) {
						...
						rows.forEach(function( _value ) {
							...
							if(tmpDateTime != strDateTime && tmp_count != 0){
								i++;
								res.data[index] = [tmpDateTime];
								for(var n = 0; n < dbcfg.table_vod.length; n++)
								{
									all_count = Number(all_count)+Number(tmp_arr[n][2]);
								}
								for(var m =0;m<3;m++)
					...
					logs.logger.error("query table_vod: "+err);
					res.result = "failed";
				}
				logs.logger.info("result:"+JSON.stringify( res ));
				thisObj.responseDirect(200, "text/json", JSON.stringify(res));
...
*/
exports.query_table_termid = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		if(useTable.length == 0)
		{
			logs.logger.error("query_table_termid->tables error")
		}
		
		var selectElement = "sum( cdn_size) cdn_size ,sum( p2p_pc_size) p2p_pc_size ,"+
		"sum( p2p_tv_size) p2p_tv_size, sum( p2p_box_size) p2p_box_size ,"+
		"sum( p2p_mo_size) p2p_mo_size, sum( lpsize) lpsize ,"+
		"sum( share_rep_count)share_rep_count , termid";
		...
		"T_m",//
		{
			...
			"_toDate":args["to"],//
			"_fromDate":args["from"],
			"table_termid":[args["table_termid"],"table_termid"]
		},//where
		selectElement,//
		"time_stamp",
		function(mysql){
							...
							if(args["table_termid"] == 1)
							{
								...
								switch(termid)
								{
									case 0:
										arr = new Array();
										arr.push(allSize );
										arr.push(cdnBandwidth/3*1.488 );
										break;
									case 1:
										tmp_arr[s] = new Array();
										tmp_arr[s].push((allSize == 0?0.00:(100 * (allSize+arr[0] - cdn-arr[1])/(allSize+arr[0]))).toFixed(2)+"%");
										tmp_arr[s].push((cdnBandwidth/3*1.488+arr[1]).toFixed(2));
										break;
									case 2:
										tmp_arr[s] = new Array();
										tmp_arr[s].push((allSize == 0?0.00:(100 * (allSize - cdn)/allSize)).toFixed(2)+"%");
										tmp_arr[s].push((cdnBandwidth/3*1.488).toFixed(2));
										break;
									case 3:
										tmp_arr[s] = new Array();
										tmp_arr[s].push((allSize == 0?0.00:(100 * (allSize - cdn)/allSize)).toFixed(2)+"%");
...
exports.query_table_termid_vv = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		if(useTable.length == 0)
		{
			logs.logger.error("query_table_termid_vv->tables error")
		}
		...
		var tmp_count = 0 ;
		logs.logger.info("useTable:"+useTable);
		var selectElement = "sum( stage_rep_count) stage_rep_count ,sum( act_rep_zero) act_rep_zero , termid";
		...
		"T_m",//��ݱ����
		{
			...
			"_toDate":args["to"],//止日期
			"_fromDate":args["from"],//起日期
			"table_termid_vv":[args["table_termid_vv"],"table_termid_vv"]//版本类型
		},//where �����ж�
		selectElement,//��ʱ���ֶβ�ѯ��sql�����sum(s.p1) p1 ,p2 ,p3... 
		"time_stamp"//��ʶʱ����ֶ�
		);
			...
			if (!err) 
			{
				...
				else 
				{
					...
					//	i++;
						...
						var termid = (_value['termid']);						
						var repCount = isNaN(Number(_value['stage_rep_count']))?0:Number(_value['stage_rep_count']);
						var act_zero = isNaN(Number(_value['act_rep_zero']))?0:Number(_value['act_rep_zero']);
						...
						if(args["table_termid_vv"] == 1)
						{
							...
							{ 
								...
								case 1:
									tmp_arr[s] = new Array();
									tmp_arr[s].push(((act_zero+Number(tmp))/10000).toFixed(0));
									break;
								case 2:
									tmp_arr[s] = new Array();
									tmp_arr[s].push((act_zero/10000).toFixed(0));
									break;
								case 3:
									tmp_arr[s] = new Array();
									tmp_arr[s].push((act_zero/10000).toFixed(0));
				...
				logs.logger.error("query table_termid_vv: "+err);
				res.result = "failed";
			}
			thisObj.responseDirect(200, "text/json", JSON.stringify(res));
		});
...
exports.query_table_webp2p = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		"T_m",//
		{
			...
			"_toDate":args["to"],//
			"_fromDate":args["from"],
			"table_webp2p":[args["table_webp2p"],"table_webp2p"]
		},//where
		selectElement,//
		"time_stamp",
		function(mysql){
					...
					if (!rows.length) {
						...
						rows.forEach(function( _value ) {
							...
							if(tmpDateTime != strDateTime && tmp_count != 0){
								...
								for(var j =0;j < 4;j++)
								{
									res.data[index]=res.data[index].concat(tmp_arr[j]);
								}								
								//console.log("res.data: "+JSON.stringify( res.data ));
								s = 0;
							}					
							if(args["table_webp2p"] == 1)
							{
								tmp_count = 1;
								switch(version)
								{ 
								...
								//console.log("tmp_arr.data: "+JSON.stringify( tmp_arr ));
								s++;
							}						
							tmpDateTime = strDateTime;
						});
...
exports.query_table_webp2p_cdn = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		var selectElement = "sum( cdn_size) cdn_size,  version";
		...
		"T_m",//
		{
			...
			"_toDate":args["to"],//
			"_fromDate":args["from"],
			"table_webp2p_cdn":[args["table_webp2p_cdn"],"table_webp2p_cdn"]
		},//where
		selectElement,//
		"time_stamp"
		);
		mysql = "select  .time_stamp ,max( .cdn_size) cdn_size, .version from \( "+mysql+",  version) as    group by  .time_stamp, .version order by  .time_stamp asc ;";
		//console.log("sql:  "+mysql);
		var beginQueryTime = new Date().getTime();
		logs.logger.info("mysql:"+mysql);
		db.getClient().query(mysql, function(err, rows) {		
			...
			if (!err) {
				...
				logs.logger.error("query table_webp2p_cdn: "+err);
				res.result = "failed";
			}
			logs.logger.info("result:"+JSON.stringify( res ));
			thisObj.responseDirect(200, "text/json", JSON.stringify(res));
...
exports.query_table_webp2p_user = function(args) {
	var thisObj = this;
	var res = {};
	res.data = [];
	res.result = "success";
	...
	if (sess) {
		...
		if(useTable.length == 0)
		{
			logs.logger.error("query_table_webp2p_user->tables error")
		}
		
		var selectElement = "sum( share_rep_count) share_rep_count,  version";
		...
		"T_m",//
		{
			...
			"_toDate":args["to"],//
			"_fromDate":args["from"],
			"table_webp2p_user":[args["table_webp2p_user"],"table_webp2p_user"]
		},//where
		selectElement,//
		"time_stamp"
		);
			...
			if (!err) {
				if (!rows.length) {
					...
					rows.forEach(function( _value ) {
						var index = i;
						var strDateTime = (_value['show_time_segment']);
						var rep_count = (_value['share_rep_count']);
						var version = (_value['version']);
						...
						if(tmpDateTime != strDateTime && tmp_count != 0){
							...
							for(var j =0;j < 4;j++)
							{
								all_count = all_count+Number(tmp_arr[j]);
							}
							for(var n=0;n< 4;n++)
							{
								arr = arr.concat((100 * tmp_arr[n]/all_count).toFixed(2)+'%');
							}	
							all_count = 0;	
							arr.unshift(tmpDateTime);						
							res.data[index] = arr;
				...
				logs.logger.error("query table_webp2p_user: "+err);
				res.result = "failed";
			}
			logs.logger.info("result:"+JSON.stringify( res ));
			thisObj.responseDirect(200, "text/json", JSON.stringify(res));
...
exports.query_table_p2p_all = function(args) {
	var thisObj = this;
	//var client = dbClientG;
	var res = {};
	...
	if (sess) {
  	var tb_prefix = "share_rate_";
		...
		if(useTable.length == 0)
		{
			logs.logger.error("query_table_p2p_all->tables error")
		}
		...
		"sum(cde_tv_size) cde_tv_size, sum(cde_box_size) cde_box_size ,"+
		"sum(cde_mo_size) cde_mo_size, sum(lpsize) lpsize ,"+
		"sum(share_rep_count)share_rep_count ";
		...
		"T_m",//��ݱ����
		{
			...
			"_toDate":args["to"],//止日期
			"_fromDate":args["from"],//起日期
			"table_p2p_all":[args["table_p2p_all"],"table_p2p_all"] //UTP 或者 WEB
		},//where �����ж�
		selectElement,//��ʱ���ֶβ�ѯ��sql�����sum(s.p1) p1 ,p2 ,p3... 
		"time_stamp",
		function(mysql){
			mysql = mysql.substring(0,mysql.indexOf(';')-2);
			mysql = mysql+";";
			//console.log("sql: "+mysql);
			var beginQueryTime = new Date().getTime();
			//logs.logger.info("mysql:"+mysql);
...
exports.query_table_p2p_web = function(args) {
  var thisObj = this;
  //var client = dbClientG;
  var res = {};
  ...
  if (sess) {
  	var tb_prefix = "share_rate_";
		...
		"T_m",//��ݱ����
		{
			...
			"_toDate":args["to"],//止日期
			"_fromDate":args["from"],//起日期
			"table_p2p_web":[args["table_p2p_web"],"table_p2p_web"] //UTP 或者 WEB
		},//where �����ж�
		selectElement,//��ʱ���ֶβ�ѯ��sql�����sum(s.p1) p1 ,p2 ,p3... 
		"time_stamp",
		function(mysql){
					...
					if (!rows.length) {
						...
						rows.forEach(function( _value ) {
							...
							var allSize = cdn + pc + tv + box + mb + cde_pc + cde_tv +cde_box + cde_mb +lp;
							 ...
							 res.data[index] = [ (allSize==0?0:(100 * (allSize - cdn)/ allSize).toFixed(2)) + '%' ]
						});
					}
				}else{
					logs.logger.error("query hisdata_ptopratio: "+err);
...
exports.query_table_p2p_cde = function(args) {
  var thisObj = this;
  //var client = dbClientG;
  var res = {};
  ...
  if (sess) {
  	var tb_prefix = "share_rate_";
		...
		"T_m",//��ݱ����
		{
			...
			"_toDate":args["to"],//止日期
			"_fromDate":args["from"],//起日期
			"table_p2p_cde":[args["table_p2p_cde"],"table_p2p_cde"] //UTP 或者 WEB
		},//where �����ж�
		selectElement,//��ʱ���ֶβ�ѯ��sql�����sum(s.p1) p1 ,p2 ,p3... 
		"time_stamp",
		function(mysql){
				...
				if (!err) {
					if (!rows.length) {
						...
						rows.forEach(function( _value ) {
							...
							var rep_count = Number(_value['share_rep_count'])||0;
							var cdn = isNaN(Number(_value['cdn_size']))?0:Number(_value['cdn_size']);
							var pc = isNaN(Number(_value['cde_pc_size']))?0:Number(_value['cde_pc_size']);
							var tv = isNaN(Number(_value['cde_tv_size']))?0:Number(_value['cde_tv_size']);
							var box = isNaN(Number(_value['cde_box_size']))?0:Number(_value['cde_box_size']);
							var mb = isNaN(Number(_value['cde_mo_size']))?0:Number(_value['cde_mo_size']);
...
//		console.log("mysql:"+mysql);
		db.getClient().query(mysql, function(err, rows) {		
	        ...
	        else 
	        {
				...
				rows.forEach(function( _value ) {
              	var index = i;
					...
					var cde_size = cde_pc + cde_tv + cde_box + cde_mb;
					var allSize = cdn + pc + tv + box + mb + cde_pc + cde_tv + cde_box + cde_mb ;//+lp;
					var cdnBandwidth = cdn*8/(1024 * 1024 * 1024*select_time);
					var saveBandwidth = (allSize - cdn)*8/(1024 * 1024 * 1024*select_time);
    	 ...
    	 console.log("result:"+JSON.stringify( res.data.length ));
    	 thisObj.responseDirect(200, "text/json", JSON.stringify(res));
    });
	}else{
		res.result = "expired";
...
exports.query_upgrade_version = function(args) {
  ...
  var res = {};
	...
	if (sess) {
		...
		//console.log("query upgarde version sql:  "+mysql);
		var beginQueryTime = new Date().getTime();
		logs.logger.info("mysql:"+mysql);
		db.getClient().query(mysql, function(err, rows) {		
			res.queryTime = new Date().getTime() - beginQueryTime;
...
*/
					...
					rows.forEach(function( _value ) {
						...
						var hour = strDateTime.split(" ");
							hour = hour[1].split(":")[0];
						//if(i<10){console.log(">>>"+JSON.stringify(_value));}
						if( 0 == time_reference )
						{
