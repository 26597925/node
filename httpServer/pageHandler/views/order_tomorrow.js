oojs$.ns("com.stock.order_tomorrow");
oojs$.com.stock.order_tomorrow = oojs$.createClass(
{
	list_benchmark_head: [
		{
            ID:'ACCOUNTID',
    ...
    ,tb:null
    ,init:function(){
        $( "#order_tomorrow_tabs" ).tabs();
        $( "#order_tomorrow_tabs" ).tabs({ selected: 0 });
        // $("#order_tomorrow_tabs").tabs('select', 2);
        $("#order_tomorrow").click(this.order_tomorrow_tab1_clk);
        $("#order_tomorrow_tabs_a1").click(this.order_tomorrow_tab1_clk);
        $("#order_tomorrow_tabs_a2").click(this.order_tomorrow_tab2_clk);
        //order_tomorrow.order_tomorrow_tab1_clk();
    }

    ,order_tomorrow_tab1_clk:function(){
        order_tomorrow.load_order_tomorrow();
    }

    ,order_tomorrow_tab2_clk:function(event){
        preload.getStock();
        ...
        dictTrade.load_userAccount(function(){
            policy.load_subscribe(function () {
                ...
                order_tomorrow.appendTB_new_order_tomorrow();
                order_tomorrow.select_title = null;
                order_tomorrow.select_title = {};
                for(var i = 0; i < policy.policy_subscribe.length; i++ ){
                    order_tomorrow.parse2obj_DIRTYPE( order_tomorrow.select_title, policy.policy_subscribe[i] );
                }
                order_tomorrow.appendTB_order_tomorrow_slct();
                if(policy.policy_subscribe.length>0){
                    $(order_tomorrow.order_select1).val(parseInt(policy.policy_subscribe[0]['DIRTYPE']));
                    order_tomorrow.handler_dirtype({'data':{'value':parseInt(policy.policy_subscribe[0]['DIRTYPE'])}});
                    
                    $(order_tomorrow.order_select2).val( parseInt(policy.policy_subscribe[0]['PGROUPID']) );
                    order_tomorrow.handler_group({'data':{'value':parseInt(policy.policy_subscribe[0]['PGROUPID'])}});
                    console.log(parseInt(policy.policy_subscribe[0]['POLICYID']))
                    $(order_tomorrow.order_select3).val( policy.policy_subscribe[0]['USERID']+"_"+policy.policy_subscribe[0]['POLICYID'] ); 
                }
            })
        });
    ...
    ,appendTB_order_tomorrow_slct:function () {
        var self = this;
    ...
    ,order_tomorrow_btn_del:function(event){
        //删除
                ...
                if(event&&event.data&&event.data.data){
                    if(typeof(event.data.data)=='object'){
                        for(var ine in event.data.data){
                            event.data.data[i]=null;
                            delete event.data.data[i];
                        }
    ...
    stock:null
    ,
    order_tomorrow_btn_herf:function(event){
        //股票详情
    ...
    ,order_tomorrow_btn_modifyreback:function(event){
        //返回
        order_tomorrow.load_order_tomorrow();
    }
    ,appendTB_modify_order:function(){
        ...
        console.log('order_tomorrow appendTB_modify_order\n',JSON.stringify(drawitem_data));
        ...
        oojs$.httpPost_json('/capital',sendAccounts,function(result,textStatus,token){
            ...
            if(result.success){
                ...
                try{
                    ...
                    capitals = '';
                    console.log('order_tomorrow',err);
                }
    ...
    ,appendTB_modifyorder_flush:function(policy_head,policy_data,account_list){
        ...
        var order_tomorrow_body_div =  $('#order_tomorrow_tabs_1').empty()

        var tb = $('<table></table>', {
            'class':"display dataTable"
        }).appendTo(order_tomorrow_body_div);
        ...
        $('<input></input>',{'type':"button",value:"提交"}).appendTo(td).click(
            ...
            ,self.order_tomorrow_new_submit
            // 
        );

        $('<input></input>',{'type':"button",value:"取消"}).appendTo(td).click(
            self.order_tomorrow_btn_modifyreback
        );

    }
    ,appendTB_new_order_tomorrow:function(){
        var self = this;
        var container =$('#order_tomorrow_tabs_2');
    ...
    ,appendTB_order_tomorrow:function(){
        var self = this;

        var panel,list_head,list_body;
        panel = $('#order_tomorrow_tabs_1');

        list_head = order_tomorrow.list_benchmark_head;//order_tomorrow_list;//self.list_benchmark_head;
        list_body = [];
        var list = null;
        list = this.order_tomorrow_list;
        ...
        for(var elm = 0; elm < list.length; elm++){
            ...
            $('<input></input>',{type:"button",value:"详情"}).appendTo(div).click(
                ...
                order_tomorrow.order_tomorrow_btn_detail
            );
            ...
            if(status == "3"||status == "4"){
                ...
                {'data':list_body[elm],'scope':self},
                order_tomorrow.order_tomorrow_btn_chg
                );
            }
            btnName = "X";
            ...
            $('<input></input>',{type:"button",value:"删除"}).appendTo(div).click(
                {'data':list_body[elm],'scope':self},
                order_tomorrow.order_tomorrow_btn_del
            );
    ...
    ,handler_policy: function(event){
        ...
        dictTrade.load_userAccount(function(){
            ...
            console.log("order_tomorrow trade_list ",JSON.stringify(trade_list));
    ...
    ,appendTB_neworder_flush:function(policy_head,policy_data,account_list){
        ...
        if(self.order_tomorrow_body_div == null){
            self.order_tomorrow_body_div = $('<div id="order_tomorrow_body_div"></div>');
            self.order_tomorrow_body_div.appendTo($('#order_tomorrow_tabs_2'));
        }

        self.order_tomorrow_body_div.empty();

        var tb = $('<table></table>', {
            'class':"display dataTable"
        }).appendTo(self.order_tomorrow_body_div);
    ...
    ,order_tomorrow_new_submit:function(event){
...
var order_tomorrow = new oojs$.com.stock.order_tomorrow();
