...
// }
var localMacAddress = p2p$.com.common.String.urlEncodeNonAscii_(this.enviroment_.getLocalMacAddresses_());
var hardwareType = p2p$.com.common.String.urlEncodeNonAscii_(this.enviroment_.deviceType_);
var requestUrl = p2p$.com.common.String.format("http://{0}/query?streamid={1}&type={2}&module=cde&version={3}&geo={4}"
...
P2P_ULOG_INFO(P2P_ULOG_FMT("{0}::onHttpDownloadCompleted_ Expired http complete for tag({1}), channel({2}), ignore",this.tag_,this.http_.tag_,this.metaData_.storageId_));
return handled;
}

this.http_ = null;
P2P_ULOG_INFO(P2P_ULOG_FMT("{0}::onHttpDownloadCompleted_ tag({1}), channel({2}), response code({3}),size({4})",this.tag_,downloader.tag_, this.metaData_.storageId_, downloader.responseCode_,downloader.responseData_.length));
...
P2P_ULOG_INFO(P2P_ULOG_FMT("{0}::onHttpDownloadCompleted_ Selector redirect to({1}), total {2} redirect times ...",this.tag_,this.selectorRedirectHost_,this.selectorRedirectTimes_));
...
P2P_ULOG_INFO(P2P_ULOG_FMT("{0}::onHttpDownloadCompleted_ Query from selector successfully, gather(http://{1}), webrtc({2}), tracker(http://{3}),stun({4}),max peers({5}), urgent size({6}), channel({7}),"
+ "rtmfp({8}), websocket({9}), webrtc({10}), upload({11}), urgent upload({12}), range interval({13} s), "
+ "upload limit({14}), init throttle({15} B/s), avg throttle({16} B/s), max reserved({17} B/s)",this.tag_,this.context_.gatherServerHost_,
...
P2P_ULOG_ERROR(P2P_ULOG_FMT("{0}::parseSelectorResponse_ Parse selector response data failed: channel({1})",this.tag_,this.metaData_.storageId_));
return false;
}
...
P2P_ULOG_ERROR(P2P_ULOG_FMT("{0} Selector timeout for channel({1}), {2} try times...",this.tag_,this.metaData_.storageId_,this.selectorTryTimes_));
this.queryFromSelector_();
...
P2P_ULOG_ERROR(P2P_ULOG_FMT("{0}::createOtherProtocols_ p2pGroupId_({1}),p2pActive({2}), selectorSuccessTime_({3}),isMobileNow({4})",this.tag_,this.metaData_.p2pGroupId_,this.p2pActive_,this.selectorSuccessTime_,isMobileNow));
return true;
}
if (!this.context_.protocolWebrtcDisabled_ && !this.enviroment_.protocolWebrtcDisabled_) {
P2P_ULOG_INFO(P2P_ULOG_FMT("{0}::createOtherProtocols_ p2pGroupId_({1})",this.tag_,this.metaData_.p2pGroupId_));
