			...
			this.workers[count].on("message",function(msg){
				...
				switch( msg.type ){
					case bean.STOCKDATA:
						...
						for(var elm_time in msg.data){
							if(self.stock_list.hasOwnProperty(elm_time)){
								for(var elm_code in msg.data[elm_time]){
									if(self.stock_list.hasOwnProperty(elm_time)){
										if( self.stock_list[elm_time].hasOwnProperty(elm_code)){
											self.stock_list[elm_time][elm_code] =	msg.data[elm_time][elm_code];
										}else{
											self.stock_list[elm_time][elm_code] = {};
											self.stock_list[elm_time][elm_code] =	msg.data[elm_time][elm_code];
										}
									}else{
										self.stock_list[elm_time] = {};
										self.stock_list[elm_time][elm_code] = {};
										self.stock_list[elm_time][elm_code] =	msg.data[elm_time][elm_code];
									}
								}
				...
				for( var i = 0; i < this.processTotal; i++ ){
					// if( i==0	 ){//|| i == 1
						if(this.workers[i].channel){
							...
							console.log(i+' workers not channel');
						};
					// }
				}
